...
try:
    ...
    ratio = 0.8  # reduced the size to 70% of the input image
    ...
    car_image = imread("my_image_resized.jpg", as_grey=True)
    car_image =car_image[round(len(car_image[:,0])/3):round(len(car_image[:,0])*7/8),round(len(car_image[0,:])/4):round(len(car_image[0,:])*3/4)]
    ...
    plate_dimensions = (0.01*label_image.shape[0], 0.3*label_image.shape[0], 0.05*label_image.shape[1], 0.8*label_image.shape[1])
    ...
    im = Image.open(name)
    size =im.size   # get the size of the input image
    ratio = 0.2  # reduced the size to 70% of the input image
    reduced_size = int(size[0] * ratio), int(size[1] * ratio)     
    im_resized = im.resize(reduced_size, Image.ANTIALIAS)
    im_resized.save("my_image_resized.jpg", "JPEG")
    car_image = imread("my_image_resized.jpg", as_grey=True)
    ...
    if len(plate_objects_cordinates)==1:
        ...
        for k in range(0,len(plate_objects_cordinates)):
            ...
            plate =car_image[plate_objects_cordinates[k][0]:plate_objects_cordinates[k][2],plate_objects_cordinates[k][1]:plate_objects_cordinates[k][3]]
            plt.imshow(plate)
            threshold_value = threshold_yen(plate)
            binary_car_image = plate > threshold_value
            plate=binary_car_image*1
            a=round(1/3*plate.shape[0])
            b=round(2/3*plate.shape[0])
        ...
        for k in range(len(plate_objects_cordinates)):
            if count_tot[k][0]>6 and count_tot[k][0]<16:
                index=k        
    for k in range(len(plate_objects_cordinates)):
            AIRE.append(plate_like_objects[k].size)
            
    for k in range(len(plate_objects_cordinates)):
        if AIRE[k]>100000 and sum(AIRE)-max(AIRE)<50000:
            print('ICI')
            index=AIRE.index(max(AIRE))       
            
    if index==[]:
        print('Second essais avec un autre type de tresholding')
    Good_boy =car_image[plate_objects_cordinates[index][0]:plate_objects_cordinates[index][2],plate_objects_cordinates[index][1]:plate_objects_cordinates[index][3]]
    plt.imshow(Good_boy)
